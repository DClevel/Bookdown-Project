...
Assignment 4:  Geospatial Analysis Part I

Using the LAGOS dataset to learn how to work with spatial data.
...
#Dynamic mapviewer
mapview(subset_spatial, legend = TRUE, layer.name = 'First 100 LAGOS Lakes')
...
minnesota_lakes %>%
  arrange(-lake_area_ha) %>%
    slice(1:1000) %>%
  mapview(.,zcol = 'lake_area_ha', legend = TRUE, layer.name = 'Lakes Area (hectares)')
```
...
illinois_iowa <- states %>%
  ...
  st_transform(2163)
mapview(illinois_iowa, legend = TRUE, layer.name = 'Iowa and Illinois')
...
minnesota_iowa <- states %>%
  ...
  st_transform(2163)
mapview(minnesota_iowa, legend = TRUE, layer.name = 'Minnesota and Iowa')
```
...
ggplot(minnesota_iowa_lakes, aes(x=log10(lake_area_ha), fill=state_name)) +
    ...
    xlab('log10(lake area (in hectares))')+
...
illinois_iowa_lakes %>%
  arrange(-lake_area_ha) %>%
  slice(1:1000) %>%
  mapview(.,zcol = 'lake_area_ha', legend = TRUE, layer.name = 'Lake Area (hectares)')
