...
```{r 11 setup, include=FALSE, message = FALSE}
...
Assignment 5:  Geospatial Analysis Part II

Learning to join spatial data with water quality data and analyze relationships
...
```{r 12 data-read, message = FALSE, warning = FALSE}
...
#Make a map
mapview(mean_spatial,zcol='log10_mean_chl', legend = TRUE, layer.name = 'Mean Chlorophyl a Concentration (μg per L)')
```
...
ggplot2::ggplot(mean_values_200, aes(x = mean_secchi, y = mean_chl)) +
  geom_point() +
  ylab("Mean Chlorophyll a Concentration (μg/L)")+
  xlab("Secchi Disk Transparency (m)")
...
sdd_means <- sdd_only %>%
                        dplyr::group_by(lagoslakeid)%>%
                        dplyr::summarise(sdd_mean = mean(secchi))
```
